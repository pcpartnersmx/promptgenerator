...
export default function Page() {
  ...
  // Helper function to check if user can edit templat
  const canEditTemplate = (userRole?: UserRole) => {
  ...
  useEffect(() => {
    if (currentProject && projects.length > 0) {
      ...
      if (updatedProject && updatedProject.generatedPrompt !== currentProject.generatedPrompt) {
        console.log('Dashboard - Sincronizando currentProject con projects array:', updatedProject);
  ...
  const handleGeneratePrompt = async (formData: { [key: string]: string }) => {
    console.log('Dashboard - handleGeneratePrompt llamado con formData:', formData);
    console.log('Dashboard - currentProject:', currentProject);
    
    if (currentProject && currentProject.template) {
      try {
        console.log('Dashboard - Template actual:', currentProject.template);
        
        // First update the project with form data
        const updatedProject = await apiUpdateProject(currentProject.id, { formData });
        console.log('Dashboard - Proyecto actualizado:', updatedProject);
        setCurrentProject(updatedProject);
        
        // Then generate the prompt using the updated form data
        console.log('Dashboard - Generando prompt con formData:', formData);
        const generatedPrompt = await apiGeneratePrompt(currentProject.id, formData);
        console.log('Dashboard - Prompt generado:', generatedPrompt);
        
        // Navigate to result tab
        setCurrentTab('result');
      } catch (error) {
        console.error('Dashboard - Error en handleGeneratePrompt:', error);
        // Error is already handled in the hook
      }
    }
  };
                ...
                <div className="px-4 lg:px-6 h-full flex flex-col">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 p-1 bg-sidebar-accent/50 rounded-lg border border-sidebar-border">
                      ...
                      {canEditTemplate(session?.user?.role) && (
                        <button
                  ...
                  <div className="flex-1 min-h-0">
                    <AnimatePresence mode="wait">
                      ...
                      >
                        ...
                        {currentTab === 'template' && canEditTemplate(session?.user?.role) && (
                          <div className="h-full bg-background border border-sidebar-border rounded-lg shadow-sm">
                            <div className="p-6 h-full">
        <TemplateEditor
